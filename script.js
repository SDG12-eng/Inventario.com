...
window.switchTab = (tab) => {
    ...
    const btnA = document.getElementById('tab-btn-activos'); const btnH = document.getElementById('tab-btn-historial');
    ...
    if(tab === 'activos') { if(btnA) btnA.className = onClass; if(btnH) btnH.className = offClass; } 
    else { if(btnH) btnH.className = onClass; if(btnA) btnA.className = offClass; }
};
...
window.cargarSesion = (datos) => {
    ...
    if(infoDiv) {
        infoDiv.innerHTML = `<div class="flex flex-col items-center"><div class="w-10 h-10 bg-indigo-50 border border-indigo-100 rounded-full flex items-center justify-center text-indigo-600 mb-2"><i class="fas fa-user"></i></div><span class="font-bold text-slate-700 uppercase tracking-wide">${datos.id}</span><span class="text-[10px] uppercase font-bold text-white bg-indigo-500 px-2 py-0.5 rounded-full mt-1 shadow-sm">${datos.rol}</span></div>`;
    }
...
window.activarSincronizacion = () => {
    ...
    onSnapshot(collection(db, "grupos"), snap => {
        ...
        snap.forEach(d => {
            ...
            if(nombre !== "SERVICIOS GENERALES") {
                ...
                listHTML += `<div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm"><span class="font-bold uppercase text-slate-700">📂 ${nombre}</span><button onclick="window.eliminarDato('grupos','${d.id}')" class="w-8 h-8 rounded bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition"><i class="fas fa-trash-alt text-xs"></i></button></div>`;
            }
        });
        if(uRol === 'admin') window.renderizarSelectorGrupos(window.todosLosGrupos);
        if(document.getElementById("lista-grupos-db")) document.getElementById("lista-grupos-db").innerHTML = listHTML;
    ...
    onSnapshot(collection(db, "sedes"), snap => {
        ...
        let listHTML = '';
        snap.docs.map(d => ({id: d.id, ...d.data()})).sort((a,b) => a.nombre.localeCompare(b.nombre)).forEach(sede => {
            options += `<option value="${sede.nombre}">📍 ${sede.nombre}</option>`;
            listHTML += `<div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm"><span class="font-bold uppercase text-slate-700">📍 ${sede.nombre}</span><button onclick="window.eliminarDato('sedes','${sede.id}')" class="w-8 h-8 rounded bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition"><i class="fas fa-trash-alt text-xs"></i></button></div>`;
        });
        if(document.getElementById("sol-ubicacion")) document.getElementById("sol-ubicacion").innerHTML = options;
        if(document.getElementById("lista-sedes-db")) document.getElementById("lista-sedes-db").innerHTML = listHTML;
    });
    ...
    if(uRol === 'admin') {
        onSnapshot(collection(db, "usuarios"), snap => {
            ...
            snap.forEach(d => {
                ...
                divUsuarios.innerHTML += `<div class="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm"><div class="truncate pr-2 w-full"><div class="flex items-center gap-2"><span class="font-bold uppercase text-slate-700">${d.id}</span><span class="text-[9px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded uppercase font-bold">${u.rol}</span></div><span class="text-[9px] text-indigo-400 block font-bold uppercase mt-1 truncate"><i class="fas fa-layer-group"></i> ${gStr}</span><span class="text-[10px] text-slate-400 block mt-1"><i class="fas fa-envelope text-[9px]"></i> ${u.email || 'Sin correo'}</span></div><div class="flex gap-2 flex-shrink-0"><button onclick="window.prepararEdicionUsuario('${jsId}')" class="w-8 h-8 rounded bg-indigo-50 text-indigo-500 hover:bg-indigo-600 hover:text-white transition"><i class="fas fa-pen text-xs"></i></button><button onclick="window.eliminarDato('usuarios','${jsId}')" class="w-8 h-8 rounded bg-slate-50 text-red-400 hover:bg-red-500 hover:text-white transition"><i class="fas fa-trash-alt text-xs"></i></button></div></div>`;
            });
        });
    }
};
...
window.procesarDatosInventario = () => {
    ...
    invFiltrado.forEach(p => {
        ...
        let controls = "";
        if (isAdmin) controls = `<div class="flex gap-2"><button onclick="window.prepararEdicionProducto('${jsId}')" class="text-slate-300 hover:text-indigo-500"><i class="fas fa-cog"></i></button><button onclick="window.eliminarDato('inventario','${jsId}')" class="text-slate-300 hover:text-red-400"><i class="fas fa-trash"></i></button></div>`;
...
window.procesarDatosPedidos = () => {
    ...
    misPedidos.forEach(p => {
        let btns = "";
        if(p.estado === 'aprobado') btns = `<div class="mt-3 pt-3 border-t border-slate-50 flex justify-end gap-2"><button onclick="window.confirmarRecibido('${p.id}')" class="bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow hover:bg-emerald-600">Recibir</button><button onclick="window.abrirIncidencia('${p.id}')" class="bg-white border border-red-200 text-red-400 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-50">Reportar</button></div>`;
...
window.renderMantenimiento = () => {
    ...
    mantGrupo.forEach(m => {
        const isCompletado = m.estado === 'completado';
        const badge = isCompletado ? '<span class="badge status-recibido">Completado</span>' : '<span class="badge status-pendiente">Pendiente</span>';
        
        let actions = "";
        if (!isCompletado) {
            actions = `<button onclick="window.completarMantenimiento('${m.id}')" class="text-white bg-emerald-500 hover:bg-emerald-600 px-3 py-1.5 rounded-lg text-xs font-bold shadow mr-2"><i class="fas fa-check"></i> Hecho</button>`;
        }
        actions += `<button onclick="window.eliminarDato('mantenimiento','${m.id}')" class="text-red-400 hover:text-red-600 px-2 py-1"><i class="fas fa-trash"></i></button>`;

        tb.innerHTML += `
        <tr class="hover:bg-slate-50 border-b border-slate-50 transition ${isCompletado ? 'opacity-60' : ''}">
            <td class="p-4">${badge}</td>
            <td class="p-4 font-bold text-slate-700 uppercase text-xs">${m.equipo}</td>
            <td class="p-4 text-slate-500 text-xs font-mono font-bold">${m.fecha_programada}</td>
            <td class="p-4 text-slate-500 text-xs">${m.detalle}</td>
            <td class="p-4 text-indigo-600 text-xs font-bold uppercase">${m.responsable}</td>
...
window.abrirModalMantenimiento = () => {
    document.getElementById("mant-equipo").value = "";
    document.getElementById("mant-fecha").value = "";
    document.getElementById("mant-responsable").value = "";
    document.getElementById("mant-detalle").value = "";
    document.getElementById("modal-mantenimiento").classList.remove("hidden");
};

window.cerrarModalMantenimiento = () => { document.getElementById("modal-mantenimiento").classList.add("hidden"); };

window.guardarMantenimiento = async () => {
    const eq = document.getElementById("mant-equipo").value.trim();
    const fe = document.getElementById("mant-fecha").value;
    const re = document.getElementById("mant-responsable").value.trim();
    const de = document.getElementById("mant-detalle").value.trim();

    if(!eq || !fe) return alert("El equipo y la fecha son obligatorios.");

    try {
        await addDoc(collection(db, "mantenimiento"), { 
            equipo: eq, fecha_programada: fe, responsable: re, detalle: de, 
            estado: 'pendiente', grupo: window.grupoActivo, 
            creado_por: window.usuarioActual.id, timestamp: Date.now() 
        });
        alert("✅ Tarea de mantenimiento programada.");
        window.cerrarModalMantenimiento();
    } catch(e) { alert("Error guardando mantenimiento."); }
};

window.completarMantenimiento = async (id) => {
    if(confirm("¿Marcar esta tarea de mantenimiento como completada?")) {
        await updateDoc(doc(db, "mantenimiento", id), { estado: 'completado', fecha_completado: new Date().toLocaleString() });
    }
};


// --- 8. DASHBOARD Y GRAFICAS ---
window.renderChart = (id, labels, data, title, palette, chartInstance, setInstance) => { 
    ...
    setInstance(new Chart(ctx, { 
        ...
        data: { labels: labels, datasets: [{ label: title, data: data, backgroundColor: id === 'locationChart' ? palette : bgColors, borderColor: palette, borderWidth: 1, borderRadius: id === 'locationChart' ? 0 : 8 }] }, 
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: id === 'locationChart', position: 'bottom' } }, scales: id === 'locationChart' ? {} : { y: { beginAtZero: true } } } 
    })); 
};
...
window.actualizarDashboard = () => {
    ...
    let labelsStock = []; let dataStock = [];
    invActivo.forEach(p => { labelsStock.push(p.id.toUpperCase().substring(0,12)); dataStock.push(p.cantidad); });
...
window.procesarSolicitudMultiple = async () => {
    ...
    try {
        ...
        items.forEach(([ins, cant]) => {
            ...
            batch.set(newRef, {
                usuarioId: window.usuarioActual.id, insumoNom: ins, cantidad: cant, ubicacion: ubi, prioridad: prio, notas: notas, // Guardamos notas
                grupo: window.grupoActivo, estado: "pendiente", fecha: fechaStr, timestamp: timestamp, fecha_solicitud: fechaStr, timestamp_solicitud: timestamp, batchId: batchId
            });
        });
        
        await batch.commit(); 
        window.enviarEmailNotificacion('nuevo_pedido', { usuario: window.usuarioActual.id, sede: ubi, items: itemsData, prioridad: prio, notas: notas });
        alert("✅ Pedido Enviado Exitosamente."); 
        window.carritoGlobal = {}; document.getElementById("sol-ubicacion").value=""; document.getElementById("sol-prioridad").value="normal"; document.getElementById("sol-notas").value="";
        window.procesarDatosInventario(); 
...
window.procesarDatosFacturas = () => {
    ...
    facturasFiltradas.sort((a,b) => b.timestamp - a.timestamp).forEach(f => {
        const docLink = (f.archivo_url && f.archivo_url.trim() !== "") ? `<a href="${f.archivo_url}" target="_blank" rel="noopener noreferrer" class="text-indigo-500 hover:text-indigo-700 font-bold bg-indigo-50 px-3 py-1.5 rounded-lg text-[10px] transition inline-flex items-center gap-1 cursor-pointer"><i class="fas fa-external-link-alt"></i> Ver Doc</a>` : '<span class="text-slate-300 text-[10px] font-bold">N/A</span>';
        ...
        tf.innerHTML += `<tr class="hover:bg-slate-50 border-b border-slate-50 last:border-0 transition"><td class="p-4 text-slate-500 text-xs font-mono">${f.fecha_compra}</td><td class="p-4 font-bold text-slate-700 uppercase text-xs">${f.proveedor}</td><td class="p-4 font-black text-emerald-600 text-right text-sm">$${parseFloat(f.gasto).toFixed(2)}</td><td class="p-4 text-slate-400 uppercase text-[10px] text-center font-bold">${f.usuarioRegistro}</td><td class="p-4 text-center">${docLink}</td><td class="p-4 text-center">${btnDelete}</td></tr>`;
    });
};
...
window.confirmarEliminarFactura = async () => { const id = document.getElementById("elim-factura-id").value; const motivo = document.getElementById("elim-factura-motivo").value.trim(); if(!motivo) return alert("Debe justificar el motivo de la eliminación para la auditoría."); try { const refFactura = doc(db, "facturas", id); const snap = await getDoc(refFactura); if (snap.exists()) { const data = snap.data(); await addDoc(collection(db, "facturas_eliminadas"), { ...data, motivo_eliminacion: motivo, eliminado_por: window.usuarioActual.id, fecha_eliminacion: new Date().toLocaleString(), timestamp_eliminacion: Date.now() }); await deleteDoc(refFactura); alert("🗑️ Factura eliminada correctamente."); document.getElementById("modal-eliminar-factura").classList.add("hidden"); } } catch(e) { console.error(e); alert("Hubo un error al eliminar la factura."); } };
window.agregarProductoRapido = async () => { const n = document.getElementById("nombre-prod").value.trim().toUpperCase(); const c = parseInt(document.getElementById("cantidad-prod").value); if (n && c > 0) { const idDoc = n.toLowerCase(); const r = doc(db, "inventario", idDoc); const s = await getDoc(r); if (s.exists()) { await updateDoc(r, { cantidad: s.data().cantidad + c }); alert(`✅ Stock actualizado: ${n}`); } else { if(confirm(`"${n}" no existe. ¿Crear nuevo en ${window.grupoActivo}?`)) { await setDoc(r, { cantidad: c, grupo: window.grupoActivo }); alert(`✅ Producto creado: ${n}`); } else return; } await addDoc(collection(db, "entradas_stock"), { insumo: n, cantidad: c, grupo: window.grupoActivo, usuario: window.usuarioActual.id, fecha: new Date().toLocaleString(), timestamp: Date.now() }); document.getElementById("modal-insumo").classList.add("hidden"); document.getElementById("nombre-prod").value = ""; document.getElementById("cantidad-prod").value = ""; } else alert("Datos inválidos."); };
...
window.eliminarDato = async (coleccion, id) => { if(confirm("¿Eliminar registro permanentemente?")) await deleteDoc(doc(db, coleccion, id)); };
...
const inicializarApp = () => {
    try {
        ...
        if (typeof cloudinary !== "undefined") {
            ...
            const btnUploadFactura = document.getElementById("btn-upload-factura"); if(btnUploadFactura) { const newBtnFact = btnUploadFactura.cloneNode(true); btnUploadFactura.parentNode.replaceChild(newBtnFact, btnUploadFactura); newBtnFact.addEventListener("click", (e) => { e.preventDefault(); if(window.cloudinaryFacturasWidget) window.cloudinaryFacturasWidget.open(); }, false); }
        }
    } catch(e) {}
};
