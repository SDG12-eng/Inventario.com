...
const EMAILJS_PUBLIC_KEY = "2jVnfkJKKG0bpKN-U"; const EMAILJS_SERVICE_ID = "service_a7yozqh"; const EMAILJS_TEMPLATE_ID = "template_zglatmb";
...
// ==========================================
// 2. FUNCIONES NÚCLEO (HOISTING GARANTIZADO)
// ==========================================
...
window.iniciarSesion = async function() {
    ...
    try { const snap = await getDoc(doc(db, "usuarios", user)); if (snap.exists() && snap.data().pass === pass) { window.cargarSesion({ id: user, ...snap.data() }); } else { alert("Credenciales incorrectas."); } } catch (e) { alert("Error de conexión al servidor."); }
};
...
window.renderCompras = function() {
    ...
    comprasGrupo.forEach(c => {
        ...
        let btnRecibir = ""; if (c.estado !== 'recibido' && window.tienePermiso('compras', 'gestionar')) { btnRecibir = `<button onclick="window.confirmarRecepcionCompra('${c.id}')" class="bg-emerald-500 text-white px-4 py-3 rounded-xl text-xs font-black shadow-lg hover:bg-emerald-600 mt-4 w-full transition flex items-center justify-center gap-2"><i class="fas fa-box-open text-lg"></i> Recibir Inventario Físico</button>`; }
...
window.renderListaInsumos = function() {
    ...
    invFiltrado.forEach(p => {
        const nombre = (p.id || '').toUpperCase(); const safeId = (p.id || '').replace(/[^a-zA-Z0-9]/g, '_'); const jsId = (p.id || '').replace(/'/g, "\\'"); tr++; ts += (p.cantidad || 0);
...
// RENDERIZADO DEL CATÁLOGO PARA PEDIR INSUMOS
window.renderCatalogoSolicitud = function() {
    ...
    if(invFiltrado.length === 0) { contenedor.innerHTML = `<p class="text-center text-slate-400 text-xs py-4 col-span-full">No hay insumos disponibles.</p>`; return; }
...
// RENDERIZADO DEL CARRITO DE PEDIDOS
window.renderCarritoPedidos = function() {
    ...
    Object.keys(window.carritoGlobal).forEach(idInsumo => {
        ...
        if (cant > 0) {
            ...
            const nombre = p.id.toUpperCase(); const jsId = p.id.replace(/'/g, "\\'");
            html += `<div class="flex items-center justify-between p-3 bg-indigo-50 rounded-xl border border-indigo-200 shadow-sm mb-2"><p class="font-black text-xs uppercase text-indigo-900 truncate flex-1 pr-2">${nombre}</p><div class="flex items-center gap-2 bg-white rounded-lg p-1 border border-indigo-100 flex-shrink-0"><button type="button" onclick="window.ajustarCantidad('${jsId}', -1)" class="w-7 h-7 rounded bg-slate-100 hover:bg-slate-200 font-black text-slate-600">-</button><span class="w-6 text-center font-black text-indigo-700 text-xs">${cant}</span><button type="button" onclick="window.ajustarCantidad('${jsId}', 1)" class="w-7 h-7 rounded bg-indigo-100 hover:bg-indigo-200 font-black text-indigo-700">+</button></div></div>`;
        }
    });
...
// RENDERIZADO DEL CATÁLOGO PARA COMPRAS
...
window.ajustarCantidad = function(idInsumo, delta) {
    const current = window.carritoGlobal[idInsumo] || 0;
    window.carritoGlobal[idInsumo] = Math.max(0, current + delta);
    window.renderCarritoPedidos(); // Renderiza el carrito
};
...
window.procesarSolicitudMultiple = async function() {
    const ubi = document.getElementById("sol-ubicacion").value; const prio = document.getElementById("sol-prioridad").value; const notas = document.getElementById("sol-notas").value.trim(); const items = Object.entries(window.carritoGlobal).filter(([_, c]) => c > 0);
    ...
    try {
        ...
        items.forEach(([ins, cant]) => { detalleInsumos += `- ${cant}x ${ins}\n`; batch.set(doc(collection(db, "pedidos")), { usuarioId: window.usuarioActual.id, insumoNom: ins, cantidad: cant, ubicacion: ubi, prioridad: prio, notas: notas, grupo: window.grupoActivo, estado: "pendiente", fecha: fs, fecha_iso: fIso, timestamp: ts, batchId: batchId }); });
        await batch.commit();
        if(window.adminEmailGlobal) { const mensajeAlmacen = `El usuario ${window.usuarioActual.id.toUpperCase()} ha realizado un nuevo pedido de insumos.\n\nSede destino: ${ubi}\nPrioridad: ${prio.toUpperCase()}\n\nInsumos solicitados:\n${detalleInsumos}\nNotas: ${notas || 'Ninguna'}`; window.enviarNotificacionEmail(window.adminEmailGlobal, `Nuevo Pedido de Insumos - ${window.usuarioActual.id.toUpperCase()}`, mensajeAlmacen); }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Activar botón de inicio de sesión
    ...
    // Inicializar Cloudinary de manera segura
