...
window.guardarMantenimiento = async () => {
    ...
    try {
        await addDoc(collection(db, "mantenimiento"), { equipo: eq, fecha_programada: fe, responsable: re, detalle: de, estado: 'pendiente', grupo: window.grupoActivo, creado_por: window.usuarioActual.id, timestamp: Date.now(), bitacora: [] });
        document.getElementById("modal-mantenimiento").classList.add("hidden");
    } catch(e) { alert("Error."); }
};

window.iniciarMantenimiento = async (id) => {
    if(confirm("¿Cambiar estado a EN PROCESO e iniciar el reloj?")) await updateDoc(doc(db, "mantenimiento", id), { estado: 'en_proceso', timestamp_inicio: Date.now() });
};

window.completarMantenimiento = async (id) => {
    if(confirm("¿Finalizar tarea y detener el reloj?")) await updateDoc(doc(db, "mantenimiento", id), { estado: 'completado', timestamp_completado: Date.now(), fecha_completado: new Date().toLocaleString() });
};
...
window.abrirBitacora = async (id) => {
    ...
    if(m.bitacora && m.bitacora.length > 0) {
        m.bitacora.forEach(b => {
            ...
            if(b.mediaUrl) {
                if(b.mediaUrl.match(/\.(mp4|webm|ogg)$/i)) {
                    ...
                    mediaHtml = `<a href="${b.mediaUrl}" target="_blank"><img src="${b.mediaUrl}" loading="lazy" class="max-h-32 object-contain rounded-lg mt-2 border border-slate-200 hover:opacity-80 transition"></a>`;
                }
            }
        ...
        html += `<p class="text-xs text-slate-400 italic mt-4">No hay observaciones registradas aún.</p>`;
    }
...
window.guardarBitacora = async () => {
    const id = document.getElementById("bitacora-mant-id").value;
    const txt = document.getElementById("bitacora-texto").value.trim();
    const url = document.getElementById("bitacora-media-url").value;
    
    if(!txt && !url) return alert("Escribe una observación o adjunta un archivo.");
    
    const mRef = doc(db, "mantenimiento", id);
    const mSnap = await getDoc(mRef);
    if(mSnap.exists()) {
        const bitacoraAnterior = mSnap.data().bitacora || [];
        const nuevoRegistro = {
            observacion: txt, mediaUrl: url, 
...
window.iniciarScanner = (inputIdTarget) => {
    ...
    window.html5QrcodeScanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, 
        (txt) => { window.detenerScanner(); const i = document.getElementById(inputIdTarget); if(i) { i.value = txt; window.debounceFiltrarTarjetas('lista-inventario', txt); } }, 
...
window.gestionarPedido = async (pid, accion, ins) => {
    ...
    if(accion === 'aprobar') {
        ...
        const iRef = doc(db, "inventario", ins.toLowerCase()); const iSnap = await getDoc(iRef);
...
window.confirmarIncidencia = async (dev) => { const pid = document.getElementById('incidencia-pid').value; const pRef = doc(db, "pedidos", pid); const pData = (await getDoc(pRef)).data(); if(dev){ const iRef = doc(db, "inventario", pData.insumoNom.toLowerCase()); const iSnap = await getDoc(iRef); if(iSnap.exists()) await updateDoc(iRef, { cantidad: iSnap.data().cantidad + pData.cantidad }); } await updateDoc(pRef, { estado: dev ? "devuelto" : "con_incidencia", detalleIncidencia: document.getElementById('incidencia-detalle').value }); document.getElementById('modal-incidencia').classList.add('hidden'); };
window.abrirModalInsumo = () => { document.getElementById("modal-insumo").classList.remove("hidden"); };
window.agregarProductoRapido = async () => { const n = document.getElementById("nombre-prod").value.trim().toLowerCase(); const c = parseInt(document.getElementById("cantidad-prod").value); if(n && c>0){ const r = doc(db, "inventario", n); const s = await getDoc(r); if(s.exists()) await updateDoc(r, { cantidad: s.data().cantidad + c }); else await setDoc(r, { cantidad: c, grupo: window.grupoActivo }); await addDoc(collection(db, "entradas_stock"), { insumo: n, cantidad: c, grupo: window.grupoActivo, usuario: window.usuarioActual.id, fecha: new Date().toLocaleString(), timestamp: Date.now() }); document.getElementById("modal-insumo").classList.add("hidden"); } };
