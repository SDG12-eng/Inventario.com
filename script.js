...
window.actualizarDashboard = function() {
    ...
    if(panelFiltros && window.tienePermiso('dashboard', 'gestionar')) {
        if(!document.getElementById("btn-excel-dashboard")) {
            panelFiltros.insertAdjacentHTML('beforeend', `<button id="btn-excel-dashboard" onclick="window.descargarReporte()" class="bg-emerald-600 text-white px-4 py-2 rounded-lg text-xs font-bold shadow hover:bg-emerald-700 transition"><i class="fas fa-file-excel"></i></button>`);
        }
...
window.renderHistorialUnificado = function() {
    ...
    const ent = window.rawEntradas.filter(e => (e.grupo || "SERVICIOS GENERALES") === window.grupoActivo).map(e => ({
        ...
        motivo: e.motivo_edicion || 'Ingreso Almacén', tiempo: 'N/A', id: e.id
    }));
    ...
    const sal = window.cachePedidos.map(p => {
        ...
        return {
            ...
            motivo: p.notas || 'Sin notas', tiempo: tProc, id: p.id
        };
    });
...
window.renderCatalogoCompras = function() {
    const contenedor = document.getElementById("grid-insumos-compra");
    ...
    contenedor.innerHTML = invFiltrado.map(p => {
        ...
        return `<div onclick="document.getElementById('compra-insumo').value='${jsId}'; document.getElementById('compra-cant').focus();" class="p-2 border border-slate-200 rounded-lg bg-white cursor-pointer hover:bg-indigo-50 hover:border-indigo-400 transition text-[10px] font-bold truncate item-tarjeta flex justify-between items-center" title="${nombre}">
            ...
            <span class="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded text-[8px] border border-slate-200">${p.cantidad}</span>
        </div>`;
    }).join('');
};
...
window.renderCompras = function() {
    ...
    let html = "";
    const comprasGrupo = window.rawCompras.filter(c => (c.grupo || "SERVICIOS GENERALES") === window.grupoActivo).sort((a,b) => b.timestamp - a.timestamp);
    ...
    comprasGrupo.forEach(c => {
        ...
        let btnRecibir = "";
        if (c.estado !== 'recibido' && window.tienePermiso('recibir', 'gestionar')) {
...
window.procesarDatosInventario = function() {
    ...
    // Renderizado del Catálogo de Pedidos
    ...
    if(cartContainer) {
        ...
        invFiltrado.forEach(p => {
            ...
            cartHTML += `<div id="row-${safeId}" class="flex items-center justify-between p-3 rounded-xl border ${active} transition-all shadow-sm item-tarjeta">
                <div class="flex items-center gap-3 flex-1 min-w-0 pr-2">
                    ${p.imagen ? `<img src="${p.imagen}" loading="lazy" class="w-10 h-10 rounded-lg object-cover flex-shrink-0 border border-slate-200">` : `<div class="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-300"><i class="fas fa-box"></i></div>`}
                    <div class="flex-1 min-w-0">
                        <p class="font-black text-xs uppercase text-slate-800 truncate">${nombre}</p>
                        <p class="text-[10px] text-slate-500 font-bold mt-0.5">Disp: <span class="text-indigo-600">${p.cantidad}</span></p>
                    </div>
                </div>
                <div class="flex items-center gap-1 bg-white rounded-lg p-1 border border-slate-200 flex-shrink-0 shadow-sm">
                    <button type="button" onclick="window.ajustarCantidad('${jsId}', -1)" class="w-7 h-7 rounded bg-slate-100 hover:bg-slate-200 font-black text-slate-600 flex items-center justify-center transition">-</button>
                    <span id="cant-${safeId}" class="w-6 text-center font-black text-indigo-700 text-xs">${enCarro}</span>
                    <button type="button" onclick="window.ajustarCantidad('${jsId}', 1)" class="w-7 h-7 rounded bg-indigo-100 hover:bg-indigo-200 font-black text-indigo-700 flex items-center justify-center transition">+</button>
                </div>
            </div>`;
        });
        
        cartContainer.innerHTML = cartHTML || `<div class="col-span-full flex flex-col items-center justify-center py-10 text-slate-400"><i class="fas fa-boxes text-4xl mb-3 opacity-50"></i><p class="text-xs font-medium">No hay insumos disponibles para pedir.</p></div>`;
    }
    
    // Sugerencias (para entradas directas)
    ...
    if (dataList || datalistCompras) {
        let listHTML = window.rawInventario.filter(p => (p.grupo || "SERVICIOS GENERALES") === window.grupoActivo).map(p => `<option value="${(p.id || '').toUpperCase()}">`).join('');
    ...
    activosFiltrados.forEach(a => {
        ...
        <div class="bg-white p-5 rounded-2xl border-2 border-slate-100 shadow-sm hover:border-indigo-200 transition flex items-center gap-6">
            ...
            <div class="flex-1 min-w-0 grid grid-cols-2 lg:grid-cols-4 gap-4 items-center">
                ...
                <div><p class="text-[9px] font-black text-slate-400 uppercase">Ubicación</p><p class="text-xs font-bold text-slate-700 uppercase truncate"><i class="fas fa-map-marker-alt text-red-400 mr-1"></i> ${a.ubicacion || 'N/A'}</p></div>
...
window.descargarReporte = async function() {
    ...
    const entActivas = window.rawEntradas.filter(e => (e.grupo || "SERVICIOS GENERALES") === window.grupoActivo && e.timestamp >= tDesde && e.timestamp <= tHasta).sort((a,b) => b.timestamp - a.timestamp);
    ...
    const equiposActivos = window.rawActivos.filter(a => (a.grupo || "SERVICIOS GENERALES") === window.grupoActivo);
    ...
    const mantActivos = window.rawMantenimiento.filter(m => (m.grupo || "SERVICIOS GENERALES") === window.grupoActivo && m.timestamp >= tDesde && m.timestamp <= tHasta);
    ...
    const compActivas = window.rawCompras.filter(c => (c.grupo || "SERVICIOS GENERALES") === window.grupoActivo && c.timestamp >= tDesde && c.timestamp <= tHasta);
...
// ==========================================
// 9. LÓGICAS DE NEGOCIO CRUD Y MODALES
// ==========================================
...
window.agregarProductoRapido = async function() {
    ...
    if(n && c>0){
        ...
        await addDoc(collection(db, "entradas_stock"), {
            ...
            timestamp: Date.now(), motivo_edicion: "Ajuste Directo"
        });
        ...
        document.getElementById("new-prod-img-url").value = "";
        document.getElementById("new-prod-preview-img").src = "";
        document.getElementById("new-prod-preview-img").classList.add("hidden");
    }
};
...
window.abrirBitacora = function(id) {
    ...
    const m = window.rawMantenimiento.find(x => x.id === id); if(!m) return; document.getElementById("bitacora-equipo-titulo").innerText = m.equipo.toUpperCase();
...
window.guardarUsuario = async function() {
    ...
    const r = document.getElementById("new-role") ? document.getElementById("new-role").value.trim().toUpperCase() : "USUARIO BASE";
