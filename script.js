...
// ==========================================
// 2. FUNCIONES NÚCLEO Y UTILIDADES
// ==========================================
...
window.enviarNotificacionEmail = async function(correoDestino, asunto, mensaje) { try { await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { to_email: correoDestino, subject: asunto, message: mensaje }); console.log("Email enviado a", correoDestino); } catch (error) { console.error("Error email:", error); } };
...
// ==========================================
// 4. FIREBASE SINCRONIZACIÓN (ONSNAPSHOT)
// ==========================================
window.activarSincronizacion = function() {
    ...
    if(window.tienePermiso('compras', 'ver')) { onSnapshot(collection(db, "compras"), snap => { window.rawCompras = []; snap.forEach(ds => { window.rawCompras.push({ id: ds.id, ...ds.data() }); }); window.renderCompras(); }); }
...
// ==========================================
// 6. RENDERIZADO DE CATÁLOGOS INTERACTIVOS (PEDIR INSUMOS / COMPRAS)
// ==========================================
window.renderCatalogoSolicitud = function() {
    ...
    contenedor.innerHTML = invFiltrado.map(p => {
        ...
        // Si no hay stock
        if (p.cantidad <= 0) {
            return `<div class="flex items-center justify-between p-3 rounded-xl border border-red-100 bg-red-50/50 opacity-60">
                <div class="flex items-center gap-3 flex-1 min-w-0 pr-2">${img}<div class="flex-1 min-w-0"><p class="font-black text-xs uppercase text-slate-800 truncate line-through">${nombre}</p><p class="text-[10px] text-red-500 font-bold uppercase mt-0.5">Agotado</p></div></div><i class="fas fa-ban text-red-300 text-lg"></i>
            </div>`;
        }

        return `<div onclick="window.ajustarCantidad('${jsId}', 1)" class="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-white shadow-sm cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition item-tarjeta">
            <div class="flex items-center gap-3 flex-1 min-w-0 pr-2">${img}<div class="flex-1 min-w-0"><p class="font-black text-xs uppercase text-slate-800 truncate">${nombre}</p><p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Stock: <span class="text-indigo-600">${p.cantidad || 0}</span></p></div></div><i class="fas fa-plus-circle text-indigo-400 text-lg"></i>
        </div>`;
    }).join('');
};
...
window.renderCarritoPedidos = function() {
    ...
    Object.keys(window.carritoGlobal).forEach(idInsumo => {
        ...
        if (cant > 0) {
            ...
            // Logica visual para desactivar botón "+" si se alcanza el máximo
...
// ==========================================
// 7. RENDERIZADO DE TABLAS Y MÓDULOS DE DATOS
// ==========================================
...
window.renderCompras = function() {
    ...
    const comprasGrupo = window.rawCompras.filter(c => (c.grupo || "SERVICIOS GENERALES") === window.grupoActivo).sort((a,b) => b.timestamp - a.timestamp); const isGestor = window.tienePermiso('compras', 'gestionar');
...
window.ajustarCantidad = function(idInsumo, delta) {
    ...
    // Validar con el stock máximo disponible en tiempo real
...
window.confirmarRecepcionCompra = async function(compraId) { if(!confirm("¿Confirmas recibir la compra en físico? Esto sumará el stock al inventario general.")) return; const cRef = doc(db, "compras", compraId); try { const cSnap = await getDoc(cRef); if(!cSnap.exists()) return; const cData = cSnap.data(); const batch = writeBatch(db); for (const item of cData.items) { const iRef = doc(db, "inventario", item.insumo); const iSnap = await getDoc(iRef); if (iSnap.exists()) { batch.update(iRef, { cantidad: iSnap.data().cantidad + item.cantidad }); } else { batch.set(iRef, { cantidad: item.cantidad, precio: 0, stockMinimo: 0, grupo: window.grupoActivo }); } const hRef = doc(collection(db, "entradas_stock")); batch.set(hRef, { insumo: item.insumo, cantidad: item.cantidad, grupo: window.grupoActivo, usuario: window.usuarioActual.id, fecha: new Date().toLocaleString(), timestamp: Date.now(), motivo_edicion: `Recepción Compra: ${cData.factura || cData.proveedor}` }); } batch.update(cRef, { estado: "recibido", recibido_por: window.usuarioActual.id, fecha_recepcion: new Date().toLocaleString(), timestamp_recepcion: Date.now() }); await batch.commit(); alert("✅ Inventario actualizado con los items de la compra."); } catch(e) { console.error(e); alert("Error en la recepción."); } };
